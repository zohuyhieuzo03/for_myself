...
import { TransactionsService, AccountsService, CategoriesService, SprintsService } from "@/client"
...
function TransactionsPage() {
  ...
  const accountsList = accounts?.data?.map((acc: any) => ({ id: acc.id, name: acc.name })) || []
  const categoriesList = categories?.data?.map((cat: any) => ({ id: cat.id, name: cat.name })) || []
        ...
        {transactions?.data && transactions.data.length > 0 ? (
          <Table.Root size={{ base: "sm", md: "md" }}>
            ...
            <Table.Body>
              {transactions.data.map((transaction: any) => {
                const account = accounts?.data?.find((acc: any) => acc.id === transaction.account_id)
                const category = categories?.data?.find((cat: any) => cat.id === transaction.category_id)
                
                return (
                  <Table.Row key={transaction.id}>
                    <Table.Cell>{transaction.txn_date}</Table.Cell>
                    <Table.Cell>
                      <Badge colorPalette={transaction.type === "in" ? "green" : "red"}>
                    ...
                    <Table.Cell>
                      <Text 
                        ...
                        fontWeight="bold"
                      >
                        {transaction.type === "in" ? "+" : "-"}{transaction.amount.toLocaleString('vi-VN')} {transaction.currency}
                      </Text>
                    </Table.Cell>
          ...
          <VStack gap={4} py={8}>
            ...
            <Text color="gray.500" textAlign="center">
              No transactions found. Create your first transaction to start tracking your spending.
            </Text>
            <AddTransaction 
              accounts={accountsList}
              categories={categoriesList}
