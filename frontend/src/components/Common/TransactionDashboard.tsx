...
export default function TransactionDashboard({ 
  ...
  const processChartData = (data: Array<{ label: string; value: number }>, type: 'days' | 'months') => {
    if (type === 'days') {
    ...
    <VStack gap={6} align="stretch">
      {showStats && (
        <SimpleGrid columns={{ base: 1, md: 3 }} gap={4}>
          ...
          <Box p={4} borderWidth="1px" borderRadius="md" bg="white">
            <Text fontSize="sm" color="gray.600" mb={1}>Transaction Count</Text>
      ...
      {(showPieChart && pieChartData.length > 0) || (showMonthlyChart && monthlyData.length > 0) ? (
                  ...
                  <VStack align="start" gap={1}>
                    ...
                    <HStack gap={2}>
                      <Text fontSize="xs" color="gray.500">
                        {new Date(transaction.received_at || transaction.date).toLocaleDateString()}
                      </Text>
