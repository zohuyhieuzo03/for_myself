      ...
      <VStack gap={6} align="stretch">
        ...
        <Box>
          ...
          <HStack gap={4}>
            <select
              ...
              style={{ maxWidth: "200px", padding: "8px", borderRadius: "4px", border: "1px solid #ccc" }}
            >
        ...
        <SimpleGrid columns={{ base: 1, md: 3 }} gap={6}>
          <Box p={6} bg="white" borderRadius="lg" shadow="sm" border="1px" borderColor="gray.200">
            <Box>
              <Box fontSize="sm" color="gray.600" mb={1}>Total Income</Box>
          ...
          <Box p={6} bg="white" borderRadius="lg" shadow="sm" border="1px" borderColor="gray.200">
            <Box>
              <Box fontSize="sm" color="gray.600" mb={1}>Total Expenses</Box>
          ...
          <Box p={6} bg="white" borderRadius="lg" shadow="sm" border="1px" borderColor="gray.200">
            <Box>
              <Box fontSize="sm" color="gray.600" mb={1}>Net Balance</Box>
              <Box fontSize="2xl" fontWeight="bold" color={summary?.net_amount && summary.net_amount >= 0 ? "green.500" : "red.500"}>
                {isLoading ? "..." : formatCurrency(summary?.net_amount || 0)}
              </Box>
              <Box fontSize="sm" color="gray.500">
                {summary?.net_amount && summary.net_amount >= 0 ? "Surplus" : "Deficit"}
              </Box>
            </Box>
          </Box>
        </SimpleGrid>
        ...
        {/* Account Breakdown */}
        {summary?.account_breakdown && Object.keys(summary.account_breakdown).length > 0 && (
          <Box p={6} bg="white" borderRadius="lg" shadow="sm" border="1px" borderColor="gray.200">
            <Heading size="md" mb={4}>Balance by Account</Heading>
            <Table.Root>
              <Table.Header>
                <Table.Row>
                ...
                {Object.entries(summary.account_breakdown)
                  ...
                  .map(([account, amount]) => (
                    <Table.Row key={account}>
                      ...
                      <Table.Cell textAlign="right" color={(amount as number) >= 0 ? "green.500" : "red.500"}>
                        {formatCurrency(amount as number)}
                      </Table.Cell>
                    </Table.Row>
                  ))}
          ...
          <Box p={6} bg="white" borderRadius="lg" shadow="sm" border="1px" borderColor="gray.200">
            ...
            <VStack gap={4} align="stretch">
              ...
              {detailedReport.incomes && detailedReport.incomes.length > 0 && (
                <Box>
                  <Heading size="sm" mb={2}>Income</Heading>
                  <Table.Root size="sm">
                    <Table.Header>
                      <Table.Row>
                    ...
                    <Table.Body>
                      {detailedReport.incomes.map((income: any) => (
                        <Table.Row key={income.id}>
                          <Table.Cell>{new Date(income.received_at).toLocaleDateString('vi-VN')}</Table.Cell>
              ...
              {/* Transactions */}
              {detailedReport.transactions && detailedReport.transactions.length > 0 && (
                <Box>
                  <Heading size="sm" mb={2}>Transactions</Heading>
                  <Table.Root size="sm">
                    <Table.Header>
                      <Table.Row>
                        <Table.ColumnHeader>Date</Table.ColumnHeader>
                    ...
                    <Table.Body>
                      {detailedReport.transactions.map((txn: any) => (
                        <Table.Row key={txn.id}>
                          <Table.Cell>{new Date(txn.txn_date).toLocaleDateString('vi-VN')}</Table.Cell>
                          <Table.Cell>
                            <Badge colorPalette={txn.type === 'in' ? 'green' : 'red'}>
                              {txn.type === 'in' ? 'Income' : 'Expense'}
                            </Badge>
                          </Table.Cell>
                          <Table.Cell>{txn.category?.name || '-'}</Table.Cell>
                          <Table.Cell>{txn.note || '-'}</Table.Cell>
                          <Table.Cell textAlign="right" color={txn.type === 'in' ? 'green.500' : 'red.500'}>
